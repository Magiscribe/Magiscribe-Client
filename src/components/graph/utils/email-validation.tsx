...
export function useValidateEmailListInput(existingEmails: string[],  validationErrors?: EmailValidationErrors): IUseValidateEmailListInput {
      ...
      React.useEffect(() => {
        ...
        if (!!emailInput) {
          if (!isValidEmailFormat(emailInput)) {
            setEmailInputError(validationErrors?.INVALID_EMAIL_INPUT_ERROR ?? DEFAULT_EMAIL_VALIDATION_ERRORS.INVALID_EMAIL_INPUT_ERROR);
          }
          else if (isEmailInList(emailInput, existingEmails)) {
            setEmailInputError(validationErrors?.DUPLICATE_USER_ERROR ?? DEFAULT_EMAIL_VALIDATION_ERRORS.DUPLICATE_USER_ERROR);
          }
        }
